-- ServiceCatalogs Module - Permissions
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.schemata WHERE schema_name = '{{SCHEMA_NAME}}') THEN
        ...
        ALTER DEFAULT PRIVILEGES FOR ROLE {{ROLE_NAME}}_owner IN SCHEMA {{SCHEMA_NAME}} GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO {{ROLE_NAME}};
        ALTER DEFAULT PRIVILEGES FOR ROLE {{ROLE_NAME}}_owner IN SCHEMA {{SCHEMA_NAME}} GRANT USAGE, SELECT ON SEQUENCES TO {{ROLE_NAME}};
        
        -- Set default privileges for app role on objects created by schema owner
        ALTER DEFAULT PRIVILEGES FOR ROLE {{ROLE_NAME}}_owner IN SCHEMA {{SCHEMA_NAME}} GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO {{APP_ROLE_NAME}};
        ALTER DEFAULT PRIVILEGES FOR ROLE {{ROLE_NAME}}_owner IN SCHEMA {{SCHEMA_NAME}} GRANT USAGE, SELECT ON SEQUENCES TO {{APP_ROLE_NAME}};
