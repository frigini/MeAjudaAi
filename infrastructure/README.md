...
**Security Notes**: 
- **ALL password environment variables are required for ALL environments** (no defaults provided)
- Required passwords: `POSTGRES_PASSWORD`, `KEYCLOAK_DB_PASSWORD`, `KEYCLOAK_ADMIN_PASSWORD`, `RABBITMQ_PASS`, `PGADMIN_DEFAULT_PASSWORD`
- The compose files will fail to start if these variables are not provided (intentional security design)
