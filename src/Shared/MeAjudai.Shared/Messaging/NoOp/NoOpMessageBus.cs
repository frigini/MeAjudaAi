...
public class NoOpMessageBus(ILogger<NoOpMessageBus> logger) : IMessageBus
{
    ...
    {
        logger.LogDebug("NoOpMessageBus: Ignoring message of type {MessageType} to queue {QueueName}",
    ...
    {
        logger.LogDebug("NoOpMessageBus: Ignoring event of type {EventType} to topic {TopicName}",
