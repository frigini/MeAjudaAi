...
public sealed class KeycloakPermissionResolver : IKeycloakPermissionResolver
{
    ...
    public async Task<IReadOnlyList<EPermission>> ResolvePermissionsAsync(string userId, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            _logger.LogDebug("Resolved {PermissionCount} permissions from {RoleCount} Keycloak roles for user {UserId}",
                permissions.Count, userRoles.Count, userId);

            return permissions.ToList();
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to resolve permissions from Keycloak for user {UserId}", userId);
            return Array.Empty<EPermission>();
        }
    }
    ...
    public async Task<IReadOnlyList<string>> GetUserRolesFromKeycloakAsync(string userId, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (userInfo == null)
            {
                _logger.LogWarning("User {UserId} not found in Keycloak", userId);
            ...
            _logger.LogDebug("Retrieved {RoleCount} roles from Keycloak for user {UserId}", userRoles.Count, userId);
        ...
        {
            _logger.LogError(ex, "Error retrieving roles from Keycloak for user {UserId}", userId);
            throw;
        }
    }
    ...
    private async Task<KeycloakUser?> GetUserInfoAsync(string userId, string adminToken, CancellationToken cancellationToken)
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Get, $"{endpoint}?username={userId}&exact=true");
    ...
    {
        _httpClient.DefaultRequestHeaders.Add("User-Agent", "MeAjudaAi-PermissionResolver/1.0");
