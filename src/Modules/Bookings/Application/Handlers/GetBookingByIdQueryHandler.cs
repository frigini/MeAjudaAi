...
public sealed class GetBookingByIdQueryHandler(
    ...
    ILogger<GetBookingByIdQueryHandler> logger) : IQueryHandler<GetBookingByIdQuery, Result<ModuleBookingDto>>
{
    public async Task<Result<ModuleBookingDto>> HandleAsync(GetBookingByIdQuery query, CancellationToken cancellationToken = default)
    {
        ...
        if (booking == null)
        {
            return Result<ModuleBookingDto>.Failure(Error.NotFound("Agendamento não encontrado."));
        }
        ...
        if (tz == null)
        {
            ...
            return Result<ModuleBookingDto>.Failure(Error.Internal("Erro ao processar fuso horário do agendamento."));
        }
