...
public class ProviderDeletedIntegrationEventHandlerTests
{
    ...
    private readonly Mock<ILogger<ProviderDeletedIntegrationEventHandler>> _loggerMock = new();
    ...
    public async Task ProviderDeletedHandler_OnSuccess_ShouldCallRemoveProviderAndLogSuccess()
    {
        ...
        var handler = new ProviderDeletedIntegrationEventHandler(_searchApiMock.Object, _loggerMock.Object);
        ...
        VerifyLog(_loggerMock, LogLevel.Information, $"Handling ProviderDeletedIntegrationEvent for provider {providerId}", Times.Once());
        VerifyLog(_loggerMock, LogLevel.Information, $"Provider {providerId} removed from search index successfully after deletion", Times.Once());
    }
    ...
    public async Task ProviderDeletedHandler_OnFailure_ShouldLogError()
    {
        ...
        // Assert
        _searchApiMock.Verify(x => x.RemoveProviderAsync(evt.ProviderId, It.IsAny<CancellationToken>()), Times.Once);
        VerifyLog(_loggerMock, LogLevel.Error, $"Failed to remove provider {providerId} from search index after deletion: {errorMessage}", Times.Once());
    }
    ...
    public async Task ProviderDeletedHandler_OnException_ShouldLogError()
    {
        ...
        // Assert
        VerifyLog(_loggerMock, LogLevel.Error, $"Error handling ProviderDeletedIntegrationEvent for provider {providerId}", Times.Once());
    }

    [Fact]
    public async Task ProviderDeletedHandler_OnCancellation_ShouldRethrow()
    {
        // Arrange
        var providerId = Guid.NewGuid();
    ...
    [Fact]
    public async Task ProviderDeletedHandler_OnHttpRequestException_ShouldLogHttpError()
    {
        // Arrange
        var providerId = Guid.NewGuid();
        _searchApiMock.Setup(x => x.RemoveProviderAsync(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
        ...
        // Assert
        VerifyLog(_loggerMock, LogLevel.Error, $"HTTP error handling ProviderDeletedIntegrationEvent for provider {providerId}", Times.Once());
    }
