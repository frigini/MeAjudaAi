...
public sealed class ServiceUpdatedDomainEventHandler(
    ...
    ILogger<ServiceUpdatedDomainEventHandler> logger) : IEventHandler<ServiceUpdatedDomainEvent>
{
    public async Task HandleAsync(ServiceUpdatedDomainEvent domainEvent, CancellationToken cancellationToken = default)
    {
        try
        {
            var service = await serviceQueries.GetByIdAsync(domainEvent.ServiceId.Value, cancellationToken);
            if (service == null)
            {
                throw new InvalidOperationException($"Service {domainEvent.ServiceId.Value} not found when handling update event.");
            }
