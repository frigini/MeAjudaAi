...
public class SubscriptionActivatedIntegrationEventHandlerTests : BaseInMemoryDatabaseTest<ProvidersDbContext>
{
    private readonly Mock<ILogger<SubscriptionActivatedIntegrationEventHandler>> _logger = new();
    private readonly Mock<IIdempotencyRepository> _idempotencyRepositoryMock = new();

    public SubscriptionActivatedIntegrationEventHandlerTests() : base(options => new ProvidersDbContext(options))
    {
    }

    [Fact]
    public async Task HandleAsync_WhenProviderExists_ShouldPromoteToGold()
    {
        ...
        var handler = new SubscriptionActivatedIntegrationEventHandler(DbContext, _idempotencyRepositoryMock.Object, _logger.Object);
