...
public class SubscriptionCanceledIntegrationEventHandlerTests : BaseInMemoryDatabaseTest<ProvidersDbContext>
{
    private readonly Mock<ILogger<SubscriptionCanceledIntegrationEventHandler>> _logger = new();
    private readonly Mock<IIdempotencyRepository> _idempotencyRepositoryMock = new();

    public SubscriptionCanceledIntegrationEventHandlerTests() : base(options => new ProvidersDbContext(options))
    {
    }
    ...
    public async Task HandleAsync_WhenProviderExists_ShouldDemoteToStandard()
    {
        ...
        var handler = new SubscriptionCanceledIntegrationEventHandler(DbContext, _idempotencyRepositoryMock.Object, _logger.Object);
    ...
    {
        var evt = new SubscriptionCanceledIntegrationEvent("Payments", Guid.NewGuid(), Guid.NewGuid());
        var correlationId = $"{evt.SubscriptionId}_{evt.Id}";
        _idempotencyRepositoryMock.Setup(x => x.IsProcessedAsync(correlationId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);
