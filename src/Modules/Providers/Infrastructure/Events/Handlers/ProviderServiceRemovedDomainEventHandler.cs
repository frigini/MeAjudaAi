...
/// <summary>
/// Handler para processar o evento de domínio quando um serviço é removido de um prestador.
/// </summary>
/// <remarks>
...
{
    public async Task HandleAsync(ProviderServiceRemovedDomainEvent domainEvent, CancellationToken cancellationToken = default)
    {
        try
        {
            logger.LogInformation(
                "Handling ProviderServiceRemovedDomainEvent for provider {ProviderId}, service {ServiceId}",
        ...
        catch (Exception ex)
        {
            logger.LogError(
                ex,
                "Error handling ProviderServiceRemovedDomainEvent for provider {ProviderId}",
                domainEvent.AggregateId);

            // Não propagamos a exceção porque o serviço já foi removido com sucesso
