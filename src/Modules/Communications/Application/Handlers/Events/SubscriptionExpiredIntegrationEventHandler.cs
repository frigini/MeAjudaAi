...
{
    private const string TemplateKey = CommunicationTemplateKeys.SubscriptionExpired;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        catch (Exception ex)
        {
            ...
            {
                logger.LogInformation(
                    "Skipping subscription expired email for user {UserId} — already enqueued (correlationId: {CorrelationId}).",
                    integrationEvent.UserId, correlationId);
                return;
            }
