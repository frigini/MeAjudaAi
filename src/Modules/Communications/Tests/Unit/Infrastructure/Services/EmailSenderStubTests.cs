...
[Trait("Category", "Unit")]
public class EmailSenderStubTests
{
    [Fact]
    public async Task SendAsync_ShouldReturnTrue_AndLog()
    {
        // Arrange
        var loggerMock = new Mock<ILogger<EmailSenderStub>>();
        var stub = new EmailSenderStub(loggerMock.Object);
        var message = new EmailMessage("test@test.com", "Subject", "<h1>Html</h1>", "Text");
