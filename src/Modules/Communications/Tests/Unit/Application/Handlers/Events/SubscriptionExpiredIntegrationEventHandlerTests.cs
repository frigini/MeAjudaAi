...
public class SubscriptionExpiredIntegrationEventHandlerTests
{
    ...
    private readonly Mock<ILogger<SubscriptionExpiredIntegrationEventHandler>> _loggerMock;
    private readonly Mock<ISerializer> _serializerMock;
    private readonly SubscriptionExpiredIntegrationEventHandler _handler;

    public SubscriptionExpiredIntegrationEventHandlerTests()
    {
        ...
        _loggerMock = new Mock<ILogger<SubscriptionExpiredIntegrationEventHandler>>();
        _serializerMock = new Mock<ISerializer>();

        _serializerMock.Setup(x => x.Serialize(It.IsAny<object>())).Returns("{}");

        _handler = new SubscriptionExpiredIntegrationEventHandler(
    ...
    public async Task HandleAsync_WhenValidEvent_ShouldEnqueueOutboxMessage()
    {
        ...
        var integrationEvent = new SubscriptionExpiredIntegrationEvent("Payments", subscriptionId, userId);
    ...
    {
        var integrationEvent = new SubscriptionExpiredIntegrationEvent("Payments", Guid.NewGuid(), Guid.NewGuid());
        _logQueriesMock.Setup(x => x.ExistsByCorrelationIdAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);
    ...
    {
        var userId = Guid.NewGuid();
        var integrationEvent = new SubscriptionExpiredIntegrationEvent("Payments", Guid.NewGuid(), userId);
        _logQueriesMock.Setup(x => x.ExistsByCorrelationIdAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(false);
        _usersModuleApiMock.Setup(x => x.GetUserByIdAsync(userId, It.IsAny<CancellationToken>()))
