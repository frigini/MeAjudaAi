...
public class GetReviewStatusQueryTests
{
    [Fact]
    public void Constructor_WithValidIds_ShouldCreateQuery()
    {
        ...
        // Act
        var query = new GetReviewStatusQuery(reviewId, correlationId);
    ...
    public void GetCacheKey_ShouldReturnCorrectKey()
    {
        // Arrange
        var reviewId = Guid.NewGuid();
        var query = new GetReviewStatusQuery(reviewId, Guid.NewGuid());

        // Act
        var cacheKey = query.GetCacheKey();

        // Assert
        cacheKey.Should().Be($"review:status:{reviewId}");
    }
    ...
    {
        // Arrange
        var query1 = new GetReviewStatusQuery(Guid.NewGuid(), Guid.NewGuid());
        var query2 = new GetReviewStatusQuery(Guid.NewGuid(), Guid.NewGuid());
    ...
    {
        // Arrange
        var query = new GetReviewStatusQuery(Guid.NewGuid(), Guid.NewGuid());
    ...
    [Fact]
    public void Query_ShouldBeQueryOfResultReviewStatusResponse()
    {
        // Arrange
        var query = new GetReviewStatusQuery(Guid.NewGuid(), Guid.NewGuid());

        // Act & Assert
        query.Should().BeAssignableTo<IQuery<Result<ReviewStatusResponse>>>();
    }
}
