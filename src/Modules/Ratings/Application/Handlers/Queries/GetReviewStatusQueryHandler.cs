...
public sealed class GetReviewStatusQueryHandler(
    ...
    ILogger<GetReviewStatusQueryHandler> logger) : IQueryHandler<GetReviewStatusQuery, Result<ReviewStatusResponse>>
{
    public async Task<Result<ReviewStatusResponse>> HandleAsync(GetReviewStatusQuery query, CancellationToken cancellationToken = default)
    {
        ...
        if (review == null)
        {
            return Result<ReviewStatusResponse>.Failure(Error.NotFound("Avaliação não encontrada."));
        }
