...
11. **Verificação de Isolamento de DB:** Garantir que o módulo não faça chamadas diretas ao banco de dados de outros módulos e que toda comunicação inter-módulos occorra exclusivamente via `IModuleApi`.
...
### 1. LocationsDbContext.GetRepository<>() — branch do InvalidOperationException não testado

O método adicionado em LocationsDbContext.cs tem dois caminhos: o happy path (retorna this) e o lançamento de InvalidOperationException quando o tipo não é suportado. O LocationsDbContextModelTests.cs só testa OnModelCreating. Nenhum teste cobre o branch de exceção.

Adicione em LocationsDbContextModelTests.cs:
...
// Classe auxiliar apenas para o teste
private class SomeUnsupportedEntity { }
```

### 2. DbContextAllowedCityQueries — normalização com null não coberta

GetByCityAndStateAsync, IsCityAllowedAsync e ExistsAsync usam `?.Trim().ToUpperInvariant() ?? string.Empty` mas nenhum teste passa null como argumento. Adicione em LocationsPersistenceIntegrationTests.cs:
...
### 3. CreateAllowedCityHandler — path de geocoding com sucesso não testado

Todos os testes existentes mockam GetCoordinatesAsync retornando null. O caminho onde o geocoding retorna coordenadas reais (e o handler as utiliza para sobrescrever o comando) não está coberto. Adicione em CreateAllowedCityHandlerTests.cs:
...
public async Task HandleAsync_WhenGeocodingThrows_ShouldStillCreateCity()
{
    ...
    result.IsSuccess.Should().BeTrue();
    capturedCity.Should().NotBeNull();
    // Deve manter as coordenadas do comando quando geocoding falha
