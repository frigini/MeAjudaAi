...
namespace MeAjudaAi.Shared.Tests.Unit.Database.Context;
...
public class BaseDbContextTests
{
    ...
    private class TestAggregate(TestId id) : AggregateRoot<TestId>(id)
    {
        public void TriggerEvent() => AddDomainEvent(new TestDomainEvent(Guid.NewGuid()));
    }

    private class TestDbContext(DbContextOptions options, IDomainEventProcessor processor) : BaseDbContext(options, processor)
    {
