...
{
    private readonly NewtonsoftJsonMessageSerializer _sut = new();
    ...
    [Fact]
    public void Deserialize_DictionaryWithObjectValues_ShouldHaveJTokenValues()
    {
        // Arrange
        ...
        // Assert
        deserialized.Should().NotBeNull();
        // Newtonsoft by default deserializes objects into JObject/JToken when T is object
        deserialized!["string"].Should().Be("value");
        deserialized["number"].Should().Be(123L); // Newtonsoft uses long for integers by default
        deserialized["bool"].Should().Be(true);
        deserialized["nested"].Should().BeOfType<JObject>();
    }
    ...
    public void FailedMessageInfo_RoundTrip_ShouldPreserveOriginalMessageAsString()
    {
        ...
        deserialized.MessageHeaders["trace-id"].Should().Be("trace-456");
    }
