...
namespace MeAjudaAi.Integration.Tests.Modules.Users.Api;
...
public class UsersApiTests : BaseApiTest
{
    ...
    public async Task UpdateUserProfile_WithValidData_ShouldReturnOk()
    {
        ...
        var createRequest = new
        {
            username = $"update_{Guid.NewGuid():N}"[..20],
        ...
        var createResponse = await Client.PostAsJsonAsync("/api/v1/users", createRequest);
        createResponse.StatusCode.Should().Be(HttpStatusCode.Created);
        var userId = GetResponseData(await ReadJsonAsync<JsonElement>(createResponse.Content)).GetProperty("id").GetGuid();
    ...
    public async Task DeleteUser_WithValidId_ShouldReturnNoContent()
    {
        ...
        var createRequest = new
        {
            username = $"delete_{Guid.NewGuid():N}"[..20],
            email = $"delete_{Guid.NewGuid():N}@example.com",
            firstName = "ToDelete",
            lastName = "User",
            password = "Password123!",
