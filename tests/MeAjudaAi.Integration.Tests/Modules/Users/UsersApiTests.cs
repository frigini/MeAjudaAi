...
public class UsersApiTests : BaseApiTest
{
    ...
    public async Task GetUsers_WithSearchTerm_ShouldFilterResults()
    {
        ...
        var otherUsername = $"zzzother_{uniqueId}";
        var otherEmail = $"zzzother_{uniqueId}@example.com";

        // Cria usuário alvo da busca
        var createResponse = await Client.PostAsJsonAsync("/api/v1/users", new
        {
            username = username,
            email = email,
        ...
        // Cria segundo usuário que NÃO deve aparecer na busca
        var otherResponse = await Client.PostAsJsonAsync("/api/v1/users", new
        {
        ...
        var otherContent = await ReadJsonAsync<JsonElement>(otherResponse.Content);
        ...
        users.Should().NotBeEmpty("Busca deve retornar pelo menos um resultado");
        var foundIds = users
            .Where(u => u.TryGetProperty("id", out _))
            .Select(u => u.GetProperty("id").GetGuid())
            .ToList();
        foundIds.Should().Contain(createdUserId,
            "O resultado da busca deve conter o usuário criado");

        // Verifica que o segundo usuário NÃO aparece nos resultados
        foundIds.Should().NotContain(otherUserId,
            "Usuário não relacionado não deve aparecer na busca por outro termo");

        // Cleanup
        await Client.DeleteAsync($"/api/v1/users/{createdUserId}");
        await Client.DeleteAsync($"/api/v1/users/{otherUserId}");
    }
