...
public class PermissionAuthorizationEndToEndTests(TestContainerFixture fixture) : BaseE2ETest<TestContainerFixture>(fixture)
{
    ...
    [Fact]
    public async Task UpdateUserDeviceToken_AsOtherUser_ShouldReturnForbidden()
    {
        // Arrange
        TestContainerFixture.BeforeEachTest();
        ConfigurableTestAuthenticationHandler.ClearConfiguration();
        ...
        // Criar usuário 1
        var user1Id = await CreateTestUserAsAdminAsync("User1");
        ...
        ConfigurableTestAuthenticationHandler.ConfigureUser(
            ...
            permissions: [EPermission.UsersUpdate.GetValue()],
            roles: ["User"]
        );
    ...
    [Fact]
    public async Task UpdateProviderDeviceToken_AsOtherProvider_ShouldReturnForbidden()
    {
        // Arrange
        TestContainerFixture.BeforeEachTest();
        ConfigurableTestAuthenticationHandler.ClearConfiguration();
        
        // Autenticar como provider 1
        ConfigurableTestAuthenticationHandler.ConfigureUser(
            userId: "provider-1",
            userName: "provider1",
            email: "p1@test.com",
            permissions: [EPermission.ProvidersUpdate.GetValue()],
            roles: ["Provider"]
        );
    ...
    private async Task<string> CreateTestUserAsAdminAsync(string namePrefix)
    {
        ...
        {
            Username = $"{namePrefix.ToLower()}-{uniqueId}",
            Email = $"{namePrefix.ToLower()}-{uniqueId}@test.com",
            FirstName = "Test",
            LastName = "User",
