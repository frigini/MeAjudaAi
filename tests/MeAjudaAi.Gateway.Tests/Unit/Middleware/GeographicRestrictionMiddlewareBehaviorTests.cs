...
public class GeographicRestrictionMiddlewareBehaviorTests
{
    ...
    public async Task InvokeAsync_IbgeServiceException_FallsBackToSimpleValidation_AndBlocks()
    {
        ...
        // Simula serviço IBGE que lança uma exceção
        ...
        var context = new DefaultHttpContext();
        // Cidade "Salvador" / state "BA" NÃO é permitida em AllowedStates (somente SP é permitido)
        // simpleValidation = false → IBGE é invocado → throws → fallback para simpleValidation (false) → bloqueado
        context.Request.Headers["X-User-Location"] = "Salvador|BA";
